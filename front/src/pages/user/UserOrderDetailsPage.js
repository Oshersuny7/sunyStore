...
const getOrder = async (orderId) => {
    const { data } = await axios.get("/orders/user/" + orderId);
    return data;
}

const loadPayPalScript = (cartSubtotal, cartItems, orderId, updateStateAfterOrder) => {
    loadScript({"client-id": "AQmEC3ar6Omz_xXLHbuD-5zN4-2X6O9RmjITsFVcojkZKAYhKUgR_OU4JDhXo-Ho9euZVdy56kPDSeNg"})
    .then(paypal => {
        paypal
...
const updateOrder = async (orderId) => {
    const { data } = await axios.put("/orders/paid/" + orderId);
    return data;
}
